...
public class GameTank : MonoBehaviour
{
    ...
    [SerializeField] List<Transform> currentBox = new List<Transform>();
    [SerializeField] List<bool> boxOpening = new List<bool>();
    ...
    [SerializeField] TMP_Text betButtonTxt,betValTex,autoStopTxt;             //texto do botao de aposta, total a apostar e onde parar
    ...
    private void Start()
    {
        ...
        else wallet.UpdateAtualCoin("BNB");
    ...
    void FixedUpdate()
    {
        ...
        betButtonTxt.text = onMove ? doABet ?winBet?$"You win: x {stopValue:0.00}":$"Stop NOW: x {multiplyBetUser:0.00}" : "Wait Next Round":doABet? "Wait Start" : $"Bet : {valueToBet:0.000} {coin}";
        betValTex.text = $"{valueToBet:0.000}";
        autoStopTxt.text = $"{stopValBet:0.00}";
        walletAddressTxt.text = wallet.player.address;
        walletBalanceTxt.text = $" {wallet.player.coinBalance:0.000} {wallet.player.atualCoin}";
    }
    ...
    IEnumerator TankInicialize()
    {
        ...
        roundListObj.ForEach(x => {
            x.GetComponent<Image>().color = new Color(1f, 1f, 1f, 0.5f);
            x.SetActive(false);
        });
        roundBets.Clear();
        ...
        {   
            if (onMove && countdown<0)
            {
                ...
                roundBets.ForEach(x =>
                {
                    ...
                    if (multiplyBetUser >= x.stop)
                    {
                        x.winBet = true;
                    }
                    if (x.winBet && roundListObj.Count - 1 >= i)
                    {
                        roundListObj[i].GetComponent<Image>().color = new Color(0f, 1f, 0f, 0.5f);
                    }
                });
                if (flameCount % 20 == 0)
                {
